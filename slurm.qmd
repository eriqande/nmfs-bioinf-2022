...
- `himem` partition: Four nodes with a boatload of memory for high-memory jobs
    - `himem01` -- `himem04`
- `bionode` partition: legacy machines from an old NWFSC cluster
    - Most of us probably don't have access to this (it is a NWFSC thing I think)
...
the reqested job enters the "queue" in at WAITING state.

We can see how many jobs are running and how many are
waiting, by using the SLURM `squeue` command:

(By the way, notice that pattern: all SLURM command start with
an `s`).
...
**One thing to note**: 
- Every job is assigned a `JOBID`.
Which is a unique integer (that gets assigned successively).
- And jobs might have a job NAME.
...
- On Sedna, we are very luck to have a great support crew that
will install software for us, and also make it available for
everyone else to use.

However, software can be complicated.  Some software might
...
The command `module` and its submodules let you interact with the
software modules.
...
convenient functions will be lost.
...
To add our three convenience functions to your `.bashrc` file, first copy
the following:
...
Then type the following into your command prompt to edit your `bashrc` with `nano`:
```{.sh filename="Type this into your shell"}
nano ~/.bashrc
```

Then paste the function definitions into the editor at the bottom
of the file, and then do `cntrl-X`, `Y`, `RETURN` to save the file
and get out of `nano`.
