...
Before we dive into cluster computing the Joy of SLURM,
we have two last topics to cover.
...
Once that is done, use the arrow keys to go to the very top of the file and type
`#!/bin/bash` on it and hit return twice, so that your file in `nano` looks like this:
```{.sh filename="This is what the contents of the file in nano should look like"}
#!/bin/bash
...
is found on Unix or Linux system.
:::
...
just hold data, and files that can the run or "executed" by the
computer.  

For you bash commands in a script to run on the computer, it must be
...
the computer looks for executable scripts, we have to write it like this:
...
If you put arguments after a script on the command line, then in the
script itself:
