...
We see that `stderr` gets written immediately to `slurm-%j.out`
...
to do some fancy stuff with delayed evaluation and such, so it is hard to know
...
After running that, use your `myjobs` function to see that the `MIN_MEMORY` is `12G`
and the `TIME_LIMIT` is 1 hour.

When it is done, run `myacct` to see that it successfully completed.
...
## Last words: Never try to redirect `sbatch`'s `output` or `error` to a path that does not exist
...
```
but do so without having actually made the direcory `outputs/mydir`, then
